<?php
// filepath: c:\WF\api_integracao_ifood\ifood-integration\src\endpoints\Picking\Order\orders-id-items-uniqueId-patch.php
...
if (!$input || !is_array($input)) {
    http_response_code(400);
    echo json_encode(['error' => 'Corpo da requisição inválido. Envie um objeto JSON conforme a documentação.']);
    exit;
}
...
if ($httpCode >= 200 && $httpCode < 300) {
    ...
    echo json_encode([
        'error' => 'Não foi possível atualizar o item do pedido',
        'orderId' => $orderId,
        'uniqueId' => $uniqueId,
        'detalhe' => $response
    ]);
