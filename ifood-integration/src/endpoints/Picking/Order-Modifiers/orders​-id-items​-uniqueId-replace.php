<?php
// filepath: c:\WF\api_integracao_ifood\ifood-integration\src\endpoints\Picking\Order\orders-id-items-uniqueId-replace-post.php
...
if (!$input || !isset($input['ean']) || !isset($input['quantity'])) {
    http_response_code(400);
    echo json_encode(['error' => 'Corpo da requisição inválido. Envie {"ean": "...", "quantity": ...}']);
    exit;
}

$accessToken = getAccessToken($clientId, $clientSecret);

$url = "https://merchant-api.ifood.com.br/picking/v1.0/orders/$orderId/items/$uniqueId/replace";
...
if ($httpCode >= 200 && $httpCode < 300) {
    ...
    echo json_encode([
        'error' => 'Não foi possível substituir o item do pedido',
        'orderId' => $orderId,
        'uniqueId' => $uniqueId,
        'detalhe' => $response
    ]);
