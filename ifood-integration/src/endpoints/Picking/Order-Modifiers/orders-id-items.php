<?php
// filepath: c:\WF\api_integracao_ifood\ifood-integration\src\endpoints\Order\orders-id-items-post.php
...
if (!$input || !is_array($input)) {
    http_response_code(400);
    echo json_encode(['error' => 'Corpo da requisição inválido. Envie um array de itens conforme a documentação.']);
    exit;
}
...
if ($httpCode >= 200 && $httpCode < 300) {
    ...
    echo json_encode([
        'error' => 'Não foi possível adicionar itens ao pedido',
        'orderId' => $orderId,
        'detalhe' => $response
    ]);
}