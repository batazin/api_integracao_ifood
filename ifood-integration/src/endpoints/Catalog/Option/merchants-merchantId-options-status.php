<?php
// filepath: c:\WF\api_integracao_ifood\ifood-integration\src\endpoints\Catalog\Option\merchants-merchantId-options-status-patch.php
...
$url = "https://merchant-api.ifood.com.br/catalog/v2.0/merchants/$merchantId/options/status";
...
if ($httpCode >= 200 && $httpCode < 300) {
    echo $response ?: json_encode([
        'success' => true,
        'message' => 'Status das opções atualizado com sucesso!',
        'merchantId' => $merchantId
    ]);
} else {
    http_response_code($httpCode !== 200 ? $httpCode : 500);
    echo json_encode([
        'error' => 'Não foi possível atualizar o status das opções',
        'merchantId' => $merchantId,
        'detalhe' => $response
    ]);
}